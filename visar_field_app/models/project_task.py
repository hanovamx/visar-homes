...
class ProjectTask(models.Model):
    ...
    def _visar_reconcile_flow_markers(self):
        """Limpia los sellos de sub-fase que NO corresponden a la etapa actual:
          - llegada/espera: solo válidas mientras está En camino;
          - inicio de servicio: solo válido En ejecución (se conserva en Completado
            como historial del timesheet).
        El `write` resultante no toca `stage_id`, así que no reentra en el override."""
        self.ensure_one()
        ...
        stage = self.stage_id
        clear = {}
        if not (s1 and stage == s1):
            clear.update({
                'visar_arrived_at': False,
                'visar_waiting_start': False,
                'visar_waiting_minutes': 0,
            })
        if not ((s2 and stage == s2) or (s3 and stage == s3)):
            clear['visar_service_start'] = False
        if clear:
            self.write(clear)
